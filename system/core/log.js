...
(function f_log() {
    ...
    = me.Log = function c_log_Log($requestState) {
        ...
        = this.log = function ($level, $str, $toDB) {
            ...
            if ((level == 1 && $level > 300) || (level != 1 && level <= $level / 100)) {
                ...
                if (level == 1) {
                    ...
                    libs.schedule.sendSignal('SHPSError', $level, $str, trace);
                }
                else {
                    
                    libs.schedule.sendSignal('SHPSError', $level, $str);
                }
            }
        ...
        console.log = function ($str) {
            ...
            libs.gLog.writeWarning('Output has to be done via SHPS\' logging module!');
            libs.gLog.writeHint('If you are a plugin author, use for example require(\'perf.js\').gLog().');
            libs.gLog.writeHint('If you are a plugin user and downloaded this module from the official plugin repository, please contact the SHPS support.');
            libs.gLog.write($str);
        };
    };
})();
